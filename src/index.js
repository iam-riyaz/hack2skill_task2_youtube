...
try{
    ...
    .sort({[sortField]:sortOrder}).skip((page - 1) * pageSize).limit(pageSize);
...
const getYouTubeData= async()=>{
    try{
       const res=  await axios.get(`${process.env.BASE_URL}${process.env.API_KEY}&type=video&q=${SearchQuery} &maxResults=5&part=snippet`)
